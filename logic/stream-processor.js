    ...
    case "allowTrust":
      return {
        ...
        asset: normalizeAsset({
          code: operation.assetCode,
          issuer: operation.trustor,
        }),
...
function processOperation(operation, txDetails, applicationOrder) {
  ...
  normalized.id = new BigNumber(txDetails.paging_token)
    .plus(new BigNumber(applicationOrder + 1))
    .toString();
  normalized.account =
    normalized.account || operation.source || txDetails.source;
...
function parseTransaction(transaction) {
  ...
  return {
    id: txDetails.hash,
    details: txDetails,
    operations: xdrTx.operations.map((op, i) =>
      processOperation(op, txDetails, i)
    ),
