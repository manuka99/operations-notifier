...
function fetchTransactions(cursor, ledgerSequence = undefined) {
  let builder = horizon.transactions();
  ...
  return builder.order("asc").limit(transactionsBatchSize).call();
}
...
class TransactionWatcher {
  ...
  processQueue() {
    ...
    this.observer.notifier
      .createNotifications(notifications)
      .then((notifications) => {
        storage
          .updateLastIngestedTx(tx.details.paging_token)
  ...
  watch() {
    ...
    storage.getLastIngestedTx().then((cursor) => {
      this.cursor = cursor;
